...
export class Node<T> {
  ...
  search(method: string, path: string): [[T, Params][]] {
    ...
    // Precompute part offsets into original path for restPathString computation
    // This avoids parts.slice(i).join('/') which allocates a new array and string each time
      ...
      for (let j = 0, len2 = curNodes.length; j < len2; j++) {
        ...
        for (let k = 0, len3 = node.#patterns.length; k < len3; k++) {
          ...
          if (matcher instanceof RegExp) {
            // Compute restPathString using path substring instead of parts.slice(i).join('/')
            if (partOffsets === null) {
